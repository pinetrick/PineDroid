    ...
    Box(
        ...
        Column(
            modifier = Modifier
                ...
                // graphicsLayer 在 draw 阶段读取 scale，不触发 composition 重组
                .graphicsLayer {
                    scaleX = scale
                    scaleY = scale
                }
                ...
                Box(modifier = Modifier.horizontalScroll(horizontalScrollState)) {
                    Row {
                        repeat(columnCount) { columnIndex ->
                            Box(modifier = Modifier.width(initialColumnWidth)) {
                                headerComposable(columnIndex, 1f)
                            }
                        }
                    }
                }
            ...
            // 数据行 — LazyColumn 按需渲染，不在 item 内读取 scale
                ...
                items(lineCount) { rowIndex ->
                    Box(modifier = Modifier.horizontalScroll(horizontalScrollState)) {
                        Row {
                            repeat(columnCount) { columnIndex ->
                                Box(modifier = Modifier.width(initialColumnWidth)) {
                                    renderCell(rowIndex, columnIndex, 1f)
                                }
                            }
                        }
                    }
